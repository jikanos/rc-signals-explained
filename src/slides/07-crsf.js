export function initCrsfSlide() {
  ...
  const ch1Arrow = document.getElementById('crsf-ch1-arrow');
  const ch1Bin = document.getElementById('crsf-ch1-bin');
  ...
  function build() {
    const groups = [
      { count: 1, fill: '#2a4d6e', label: 'Addr' },
      { count: 1, fill: '#4a4a4a', label: 'Len' },
      { count: 1, fill: '#6e5a2a', label: 'Type' },
      { count: 16, fill: '#3a5a3a', label: 'Payload — 16 каналов × 11 бит' },
      { count: 1, fill: '#6e2a2a', label: 'CRC' },
    ];
    ...
    groups.forEach((g) => {
      ...
      for (let i = 0; i < g.count; i++) {
        const isCh1 = g.label.startsWith('Payload') && i === 0;
